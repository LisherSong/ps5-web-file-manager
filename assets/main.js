...
function opLabel(op) {
  return { copy: t("copy"), move: t("move"), delete: t("delete"), chmod: t("permissionsTitle"), download: t("download"), upload: t("upload"), pkg_install: t("installPackage"), extract: t("extract") }[op] || op;
}

function taskOpLabel(op) {
  return { copy: t("copying"), move: t("moving"), delete: t("deleting"), chmod: t("changingPermissions"), download: t("downloading"), upload: t("uploading"), pkg_install: t("installPackage"), extract: t("extracting") }[op] || op;
}
...
function handleTerminalTask(task) {
  ...
  if (task.state === "done") {
    if (task.op === "extract") {
      clearTrackedTask();
      setStatus(t("extractDone", { name: taskSubject(task) }));
      return;
    }
...
function promptLargeMode(sizeBytes) {
  return confirm(t("extractLargeAsk", { size: formatSize(sizeBytes, '-') }));
}
...
async function uploadAndExtractFile(file, relativeName, alreadyAsked) {
  ...
  try {
    ...
    const task = await apiForm("/api/upload/prepare", {
      ...
      count: 1,
      rels: rel,
      sizes: String(file.size || 0),
      overwrite: "1"
    });
    taskId = task.task_id;
    trackTask(taskId, "upload", false);
    await pollTasks();
    setStatus(t("uploadingStatus", { index: 1, count: 1, name: rel }));
    await uploadFileRequest(taskId, file, rel, true, 1);
    await api("/api/upload/finish", { task_id: taskId });
    await pollTasks();
    await startExtractTask(zipPath, cwd, conflict, true, rel);
  } catch (err) {
    const terminalAbort = err && err.name === "TerminalTaskAbort";
    const canceled = err && err.name === "AbortError";
    if (!terminalAbort) {
...
extractBtn.addEventListener("click", actionExtract);
